    ...
    upload.single('image')(req, res, async (err) => {
      ...
      if (bucket) {
        try {
          ...
          // Attempt to assign public-read permissions so anyone can load it via standard GCS direct link
          try {
            await fileRef.makePublic();
          } catch (pubErr) {
            console.log('Permission makePublic() skipped. Serving with media token/public query instead.');
          }

          // Use the secure direct Firebase Storage public URL
          const publicUrl = `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${encodeURIComponent(fileName)}?alt=media`;
          console.log('Uploaded successfully to permanent Firebase Storage:', publicUrl);
          return res.json({ url: publicUrl });
        } catch (storageErr: any) {
          console.error('INFO: GCS Storage upload failed, falling back to local files:', storageErr.message || storageErr);
        }
      }
